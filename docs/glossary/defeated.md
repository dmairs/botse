...
- If a [unit](/docs/glossary/unit) is defeated by a [status effect](/docs/battles/status-effects/) die, such as [Bane (<img src="/icons/bane.svg" alt="Bane Icon" className="icon-svg" />)](/docs/battles/status-effects/bane), it is considered defeated but does not trigger any effect that requires the unit to have been defeated by a specific other unit.
